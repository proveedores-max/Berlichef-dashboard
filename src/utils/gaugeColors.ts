...
export function getGaugeColor(value: number, thresholds: GaugeThresholds, invertColors: boolean): string {
  if (invertColors) {
    ...
    if (thresholds.danger !== undefined && value <= thresholds.danger) return GAUGE_COLORS.yellow
    return GAUGE_COLORS.red
  } else {
    if (thresholds.good !== undefined && value >= thresholds.good)    return GAUGE_COLORS.green
